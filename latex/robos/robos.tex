...
Dado um mapa de um labirinto com a posição inicial do robô e as posições de ``tesouros'' espalhados pelo labirinto, o seu programa deve imprimir quantos tesouros podem ser coletados a partir da posição inicial. Um tesouro pode ser coletado apenas se o robô consegue alcançar sua posição através de movimentos para norte, sul, leste ou oeste, em qualquer quantidade ou ordem. A borda mais externa sempre conterá paredes, evitando que o robô saia acidentalmente do labirinto.
\end{descricao}

\begin{entrada}
A entrada é composta por diversas instâncias. Cada uma das instâncias começa com dois inteiros $3 \leq m, n \leq 1000$, respectivamente o comprimento e a largura de um labirinto. As $m$ linhas seguintes descrevem o labirinto e contêm $n$ caracteres cada uma. Cada caractere `\#' representa uma parede, ou seja, uma posição que não pode ser ocupada pelo robô, enquanto cada caractere `.' (ponto) representa um espaço livre. Um caractere `\$' representa um tesouro, e o caractere `$I$' representa a posição inicial do robô. Haverá exatamente um caractere `$I$' em cada instância. A entrada termina quando $m=n=0$, e essa instância não deve ser processado.
\end{entrada}
